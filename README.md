# CodeNest — Learn. Build. Grow.

![Main Page](images/mainpage.png)

> **CodeNest** is a developer-focused ed-tech platform: project-based courses in web
> development, data and systems, taught by working engineers.

## Design system

The front end uses a dark-first theme driven by tokens in `tailwind.config.js` and
`src/App.css`. Prefer the semantic names in new code.

| Role | Token | Value |
| --- | --- | --- |
| Page background | `ink` / `richblack-900` | `#0B1120` |
| Raised background | `surface` / `richblack-850` | `#111827` |
| Cards, inputs, menus | `card` / `richblack-800` | `#151E2E` |
| Primary text | `richblack-5` | `#F8FAFC` |
| Secondary text | `richblack-100` | `#94A3B8` |
| Hairline border | `line` | `rgba(148,163,184,0.15)` |
| Primary accent | `accent` / `yellow-50` | `#6366F1` |
| Secondary accent | `accent2` / `blue-100` | `#22D3EE` |

The legacy colour *names* (`richblack`, `yellow`, `blue`, `pink`, `caribbeangreen`,
`brown`) are retained so existing classes keep resolving, but every value now maps to
the CodeNest palette — `yellow` is indigo and `blue` is cyan.

Reusable classes live in `src/App.css` under `@layer components`:

* Buttons — `cn-btn-primary`, `cn-btn-secondary`, `cn-btn-ghost`, `cn-btn-danger`,
  with `cn-btn-sm` / `cn-btn-lg` size modifiers
* Surfaces — `cn-card`, `cn-card-hover`, `cn-panel`, `cn-terminal`, `cn-grid-bg`
* Forms — `cn-input`, `cn-label`, `cn-input-error`, `cn-error-text`, `cn-help-text`
* States — `cn-empty`, `skeleton`

Type is Inter for UI and JetBrains Mono for code, terminal output and metadata.
Motion sits between 150–400ms and is disabled under `prefers-reduced-motion`.

> **Tailwind 3.2 note:** this project is pinned to Tailwind 3.2.7, which silently drops
> opacity modifiers that are not on the default scale. Use `/10`, `/20`, `/40`, `/90`
> and so on — values like `/12` or `/85` compile to nothing.


## Project Description

CodeNest is a fully functional ed-tech platform that enables users to create, consume,
and rate educational content. The platform is built using the MERN stack, which includes
ReactJS, NodeJS, MongoDB, and ExpressJS.
CodeNest aims to provide:
...
In summary, CodeNest is a versatile and intuitive ed-tech platform that is designed to
...
The CodeNest ed-tech platform consists of three main components: the front end, the
...
Here is a high-level diagram that illustrates the architecture of the CodeNest ed-tech
platform:
...
The front end of CodeNest has all the necessary pages that an ed-tech platform should
...
CodeNest uses a monolithic architecture, with the backend built using Node.js and
Express.js, and MongoDB as the primary database. 

Features and Functionalities of the Back-end: 
The back end of CodeNest provides a range of features and functionalities, including:
...
1. Cloud-based media management: CodeNest uses Cloudinary, a cloud-based media
...
The back end of CodeNest uses a range of frameworks, libraries, and tools to ensure its
...
The back end of CodeNest uses a range of data models and database schemas to
...
Overall, the back-end of CodeNest is designed to provide a robust and scalable solution
...
The CodeNest platform's API is designed following the REST architectural style. The
...
In conclusion, the REST API design for the CodeNest ed-tech platform is a crucial part
...
what kind of data it will accept or return. With this API design, CodeNest will be able to
